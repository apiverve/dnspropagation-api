...
class DnspropagationValidationException implements Exception {
  final List<String> errors;

  DnspropagationValidationException(this.errors);

  @override
  String toString() => 'DnspropagationValidationException: ${errors.join("; ")}';
}
...
/// DNS Propagation Checker API client.
///
/// For more information, visit: https://apiverve.com/marketplace/dnspropagation?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [domain] (required) - The domain name to check propagation for [format: domain]
/// * [type] - The DNS record type to check
class DnspropagationClient {
  final String apiKey;
  ...
  DnspropagationClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/dnspropagation',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DnspropagationValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DnspropagationValidationException(errors);
    }
  }

  /// Execute a request to the DNS Propagation Checker API.
  ///
  /// Parameters are validated before sending the request.
  Future<DnspropagationResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DnspropagationException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DnspropagationResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DnspropagationException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DnspropagationException('Resource not found');
      } else {
        throw DnspropagationException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DnspropagationException) rethrow;
      throw DnspropagationException('Request failed: $e');
    }
  }
...
/// Exception thrown by the DNS Propagation Checker API client.
class DnspropagationException implements Exception {
  final String message;

  DnspropagationException(this.message);

  @override
  String toString() => 'DnspropagationException: $message';
}
